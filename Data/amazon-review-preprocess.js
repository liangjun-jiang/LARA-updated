...
readInterface.on('line', (line) => {
  if (line.indexOf('[id]:') > -1) {
    ...
    productMap['<Product Name>'] = productName;
  ...
  if (line.indexOf('#####') > -1) {
    if (!products.get(productMap['<Product Name>'])) {
      let ps = [];
      ps.push(productMap);
      products.set(productMap['<Product Name>'],ps);
    } else {
      ps = products.get(productMap['<Product Name>']);
      ps.push(productMap);
      products.set(productMap['<Product Name>'],ps);
    }

    if (productMap['<Product Name>'] !== lastProductName) {
      ...
      const filename = (""+Math.random()).substring(2,9) + '.dat';
      fs.writeFile(filename, str, (err) => {
        if (err) console.log(err);
        console.log("Successfully Written to File.");
      });
      lastProductName = productMap['<Product Name>'];
    }  
